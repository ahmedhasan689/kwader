...
class EmployerSettingsRepository implements EmployerSettingsInterface
{
    /**
     * @param $id
    ...
    {
        $data = [];
        $data['employer'] = Employer::where('user_id', Auth::user()->id)->first();

        $data['user'] = User::where('id', $data['employer']->user_id)->first();
        ...
        $data['notifications'] = NotificationSystem::where('user_type', 'Employer')->get();

        return view('employer.settings.index', $data);
    }
...
//        dd($request);
    ...
    public function deleteAccount(Request $request)
    {
        ...
        $employer = Employer::where('user_id', Auth::user()->id)->delete();
    ...
    public function notificationSystem(Request $request)
    {
        ...
        $notifications = DB::table('notification_user')->where('user_id', $user->id)->get();

        if ( $notifications ) {
            foreach($notifications as $notification) {
                $nn = DB::table('notification_user')->where('user_id', $user->id)->delete();
            }

            foreach ( $request->except('_token') as $notification_system ) {
                DB::table('notification_user')->insert([
                    'user_id' => $user->id,
                    'notifications_id' => $notification_system
                ]);
            }

        }

        toastr()->success('تم تحديث نظام الإشعارات بنجاح');
