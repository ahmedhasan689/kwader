            ...
            <div class="tab-content pt-5" style="width:80%;" id="v-pills-tabContent">
                ...
                <div class="tab-pane fade show active" id="v-pills-information" role="tabpanel"
                    ...
                    <div class="row info-page">
                        <div class="col-lg-6">
                            <form action="{{ route('employee.settings.setNames') }}" method="POST" style="height:300px ;">
                                @csrf
                                @method('PUT')
                                <h4>المعلومات الشخصية</h4>
                                <div class="mb-3">
                                    <label for="exampleInputEmail1" class="form-label">الاسم</label>
                                    <input type="text" name="first_name" class="form-control @error('first_name') is-invalid @enderror" id="exampleInputEmail1" aria-describedby="emailHelp" value="{{ $employee->user->first_name }}">
                                    @error('first_name')
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="last_name" class="form-control @error('last_name') is-invalid @enderror" id="exampleInputPassword1" value="{{ $employee->user->last_name }}">
                                    @error('last_name')
                                    <span>
                                            {{ $message }}
                                        </span>
                        ...
                        <div class="col-lg-6">
                            <form action="{{ route('employee.settings.setAvatar') }}" method="POST" style="height:300px ;" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                ...
                                <a href="#" style="display: flex; align-items:center; margin-top: 50px;">
                                    <div class="image mb-5">
                                        <img src="{{ $employee->image }}" alt="" id="output">
                    ...
                    <div class="row info-page mt-3">
                        ...
                        <form action="{{ route('employee.settings.setInfo') }}" method="POST" class="info-page2">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="d-flex">
                                <div>
                                    <div class="mb-3">
                                        ...
                                        <input type="date" name="date_of_birth" class="form-control" value="{{ $employee->date_of_birth }}" id="date" aria-describedby="emailHelp">
                                    </div>
                                    <div class="mb-3">
                                        <label for="country" class="form-label">الدولة</label>
                                        <select class="form-select" aria-label="Default select" name="country_id">
                                            @foreach($countries as $country)
                                                <option @if($country->id == $employee->country->id) selected @endif value="{{ $country->id }}">
                                ...
                                <div>
                                    <div class="mb-3 ">
                                        ...
                                        <select class="form-select" aria-label="Default select" name="gender">
                                            <option @if($employee->gender == 'Male') selected @endif value="Male">ذكر</option>
                                            <option @if($employee->gender == 'Female') selected @endif value="Female">أنثى</option>
                                        </select>
                                    </div>
                                    <div class="mb-3 ">
                                        ...
                                        <select class="form-select" aria-label="Default select language" name="language_id">
                                            @foreach($languages as $language)
                                                <option @if($language->id == $employee->language->id) selected @endif value="{{ $language->id }}">
                ...
                <div class="tab-pane fade" id="v-pills-gmail" role="tabpanel" aria-labelledby="v-pills-gmail-tab"
                     tabindex="0">
                    <div class="row info-page ">
                        <form action="{{ route('employee.settings.setEmail') }}" method="POST" class="info-page2">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="d-flex">
                                <div class="mb-3">
                                    ...
                                    <input type="email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ $employee->user->email }}" id="exampleInputEmail1" aria-describedby="emailHelp">
                                    @error('email')
                                    <span class="text-danger">
                                            {{ $message }}
                                        </span>
                ...
                <div class="tab-pane fade" id="v-pills-password" role="tabpanel" aria-labelledby="v-pills-password-tab" tabindex="0">
                    <div class="row info-page">
                        <form action="{{ route('employee.settings.resetPassword') }}" method="POST" class="mb-2">
                            @csrf
                            @method('PUT')
                ...
                <div class="tab-pane fade " id="v-pills-phone" role="tabpanel" aria-labelledby="v-pills-phone-tab"
                     tabindex="0">
                    <div class="row info-page">
                        <form action="{{ route('employee.settings.setPhoneNumber') }}" method="POST">
                            @csrf
                            @method('PUT')
                            <h4>
                                رقم الجوال
                            ...
                            <div class="info-page2">
                                <div class="d-flex align-items-center">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="phone_number" class="form-control @error('phone_number') is-invalid @enderror" value="{{ $employee->user->phone_number }}" id="phone-number">
                                        @error('phone_number')
                                        <span class="text-danger">
                                                {{ $message }}
                                            </span>
                ...
                <div class="tab-pane fade mb-2" id="v-pills-delete" role="tabpanel" aria-labelledby="v-pills-delete-tab"
                     tabindex="0">
                    <div class="row info-page">
                        <form action="{{ route('employee.settings.deleteAccount') }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <h4>
                                احذف حسابي
